...
BEGIN_DHTML_EVENT_MAP(CDiskMarkDlg)
	...
	DHTML_EVENT_ONCLICK(_T("SequentialMultiQT"), OnSequentialMultiQT)
...
{
	CString cstr;
	m_SequentialMultiQueues = GetPrivateProfileInt(_T("Settings"), _T("SequentialMultiQueues"), 32, m_Ini);
	if (m_SequentialMultiQueues <= 0 || m_SequentialMultiQueues > MAX_QUEUES)
	{
		m_SequentialMultiQueues = 32;	// default value is 32.
	}

	m_SequentialMultiThreads = GetPrivateProfileInt(_T("Settings"), _T("SequentialMultiThreads"), 1, m_Ini);
	if (m_SequentialMultiThreads <= 0 || m_SequentialMultiThreads > MAX_THREADS)
	{
		m_SequentialMultiThreads = 1;	// default value is 1.
	}

	m_RandomMultiQueues = GetPrivateProfileInt(_T("Settings"), _T("RandomMultiQueues"), 32, m_Ini);
	if (m_RandomMultiQueues <= 0 || m_RandomMultiQueues > MAX_QUEUES)
	{
		m_RandomMultiQueues = 32;	// default value is 32.
	}

	m_RandomMultiThreads = GetPrivateProfileInt(_T("Settings"), _T("RandomMultiThreads"), 1, m_Ini);
	if (m_RandomMultiThreads <= 0 || m_RandomMultiThreads > MAX_THREADS)
	{
		m_RandomMultiThreads = 1;	// default value is 1.
	}
}
...
void CDiskMarkDlg::InitScore()
{
	...
	m_SequentialReadMultiQTScore = 0.0;
	m_SequentialWriteMultiQTScore = 0.0;
	m_RandomRead4KBMultiQTScore = 0.0;
	m_RandomWrite4KBMultiQTScore = 0.0;
...
void CDiskMarkDlg::UpdateScore()
{
	...
	SetMeter(_T("SequentialReadMultiQT"), m_SequentialReadMultiQTScore);
	SetMeter(_T("SequentialWriteMultiQT"), m_SequentialWriteMultiQTScore);

	SetMeter(_T("RandomRead4KBMultiQT"), m_RandomRead4KBMultiQTScore);
	SetMeter(_T("RandomWrite4KBMultiQT"), m_RandomWrite4KBMultiQTScore);
	...
	cstr.Format(_T("%8.1f IOPS"), m_RandomRead4KBMultiQTScore * 1000 * 1000 / 4096);
	SetElementPropertyEx(_T("RandomRead4KBMultiQT"), DISPID_IHTMLELEMENT_TITLE, cstr);
	cstr.Format(_T("%8.1f IOPS"), m_RandomWrite4KBMultiQTScore * 1000 * 1000 / 4096);
	SetElementPropertyEx(_T("RandomWrite4KBMultiQT"), DISPID_IHTMLELEMENT_TITLE, cstr);
}
...
HRESULT CDiskMarkDlg::OnSequentialMultiQT(IHTMLElement* /*pElement*/)
{
	if (m_WinThread == NULL)
	{
		UpdateData(TRUE);
		...
		UpdateScore();
		m_DiskBenchStatus = TRUE;
		m_WinThread = AfxBeginThread(ExecDiskBenchSequentialMultiQT, (void*)this);
		if (m_WinThread == NULL)
		{
			m_DiskBenchStatus = FALSE;
		}
...
HRESULT CDiskMarkDlg::OnRandom4KBMultiQT(IHTMLElement* /*pElement*/)
{
	if(m_WinThread == NULL)
	{
		UpdateData(TRUE);
		...
		UpdateScore();
		m_DiskBenchStatus = TRUE;
		m_WinThread = AfxBeginThread(ExecDiskBenchRandom4KBMultiQT, (void*)this);
		if(m_WinThread == NULL)
		{
			m_DiskBenchStatus = FALSE;
		}
...
void CDiskMarkDlg::ChangeButtonStatus(BOOL status)
{
	if(status)
	{
		...
		ChangeButton(_T("Random4KB"),		_T("button1"),	_T("Random 4KiB"),				_T("4K"));

		toolTip.Format(L"Sequential, Queues=%d, Threads=%d", m_SequentialMultiQueues, m_SequentialMultiThreads);
		title.Format(L"Seq<br>Q%dT%d", m_SequentialMultiQueues, m_SequentialMultiThreads);
		ChangeButton(_T("SequentialMultiQT"), _T("button2"), toolTip, title);

		toolTip.Format(L"Random 4KiB, Queues=%d, Threads=%d", m_RandomMultiQueues, m_RandomMultiThreads);
		title.Format(L"4K<br>Q%dT%d", m_RandomMultiQueues, m_RandomMultiThreads);
		ChangeButton(_T("Random4KBMultiQT"), _T("button2"), toolTip, title);
	...
	else
	{
		...
		ChangeButton(_T("SequentialMultiQT"), _T("button1"), _T("Stop"), _T("Stop"));
		ChangeButton(_T("Random4KBMultiQT"), _T("button1"), _T("Stop"), _T("Stop"));
...
\r\n\
%SequentialReadMultiQT%\r\n\
%SequentialWriteMultiQT%\r\n\
%RandomRead4KBMultiQT%\r\n\
%RandomWrite4KBMultiQT%\r\n\
%SequentialRead%\r\n\
%SequentialWrite%\r\n\
%RandomRead4KB%\r\n\
%RandomWrite4KB%\r\n\
...
");
	...
	clip.Replace(_T("%SequentialReadMultiQT%"), cstr);
	cstr.Format(_T("  Sequential Write (Q=%3d,T=%2d) : %9.3f MB/s"), m_SequentialMultiQueues, m_SequentialMultiThreads, m_SequentialWriteMultiQTScore);
	clip.Replace(_T("%SequentialWriteMultiQT%"), cstr);

	cstr.Format(_T("  Random Read 4KiB (Q=%3d,T=%2d) : %9.3f MB/s [%8.1f IOPS]"), m_RandomMultiQueues, m_RandomMultiThreads, m_RandomRead4KBMultiQTScore, m_RandomRead4KBMultiQTScore * 1000 * 1000 / 4096);
	clip.Replace(_T("%RandomRead4KBMultiQT%"), cstr);
	cstr.Format(_T(" Random Write 4KiB (Q=%3d,T=%2d) : %9.3f MB/s [%8.1f IOPS]"), m_RandomMultiQueues, m_RandomMultiThreads, m_RandomWrite4KBMultiQTScore, m_RandomWrite4KBMultiQTScore * 1000 * 1000 / 4096);
	clip.Replace(_T("%RandomWrite4KBMultiQT%"), cstr);

	cstr.Format(_T("         Sequential Read (T= 1) : %9.3f MB/s"), m_SequentialReadScore);
	clip.Replace(_T("%SequentialRead%"), cstr);
	cstr.Format(_T("        Sequential Write (T= 1) : %9.3f MB/s"), m_SequentialWriteScore);
	clip.Replace(_T("%SequentialWrite%"), cstr);

	cstr.Format(_T("   Random Read 4KiB (Q= 1,T= 1) : %9.3f MB/s [%8.1f IOPS]"), m_RandomRead4KBScore, m_RandomRead4KBScore * 1000 * 1000 / 4096);
	clip.Replace(_T("%RandomRead4KB%"), cstr);
	cstr.Format(_T("  Random Write 4KiB (Q= 1,T= 1) : %9.3f MB/s [%8.1f IOPS]"), m_RandomWrite4KBScore, m_RandomWrite4KBScore * 1000 * 1000 / 4096);
